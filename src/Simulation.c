...
// Atom* ss->atom_arr[]; // array containing all atoms in the simulation
// double ss->elapsed_stime = 0; 
// bool ss->evaporation_flag = true;
...
// int ss->rate_cnt; // number of rates in ss->rate_arr list (filled indices)
...
// ENHANCE: malloc?
// Rate ss->rate_arr[MAXIMUM_NUMBER_OF_ACTIVATION_BARRIERS]; // array containing all the unique rate constants and count of atoms that have that k and indices in ss->transition_arr
// Rates default initalized to all zeros values?
// Transition *ss->transition_arr[MAXIMUM_NUMBER_OF_CONCURRENT_TRANSITIONS]; // array of the possible atom transitions (atom index + jump offset index)
//Transition ss->transition_arr[MAXIMUM_NUMBER_OF_CONCURRENT_TRANSITIONS];
// contains all the same information in ss->atom_arr[i].transition_indices
...
unsigned long perform_simulation(struct SimulationState *ss, struct SimulationEnv *se) //potentially FILE* as arguments
{
	...
	{
		do_initialize_simulation(ss->simulation_type);
		ss->elapsed_stime = 0.0;
	}*/ 
	...
	for (i=0;i<ss->atom_cnt;++i)	
		refresh_transitions(i, ss, se);			// resets all kinetic paramters
	...
	while (!simulation_end)
	{
		...
		if (ss->simulation_should_kill_itself) // abort simulation (only happens if atoms overlap)
		{
			...
			organize(ss->atom_arr, ss->atom_cnt); //replace the copy with draw, maybe not needed
		...
		while (moved_flag == false) {
						...
						&& (transition_type_probability < ss->transition_probability.ubound[j]))
			{
				...
				// which_one gives the location of the ss->transition_arr, which gives
				...
				if (jump_vector != se->max_neighbors) //diffusion
				{
					...
					g_lastxt = ss->atom_arr[atom_number]->lattice[0] + jump_offset[jump_vector].dx;
					g_lastyt = ss->atom_arr[atom_number]->lattice[1] + jump_offset[jump_vector].dy;
					g_lastzt = ss->atom_arr[atom_number]->lattice[2] + jump_offset[jump_vector].dz;
					...
					natn = add_atom(g_lastxt, g_lastyt, g_lastzt, atype, NORMAL, ss, se);
		...
		if (moved_flag == false) //only happens iff jump_vector == se->max_neighbors
		{
			...
			/*if (deposition_type == DEPOSITION_TYPE_RAINFALL)
			{
				...
				natn = add_atom(dep_x, dep_y, dep_z, atype, NORMAL, se);
			}
			else if (deposition_type == DEPOSITION_TYPE_RANDOM_WALKER)
			{
				...
				do
				{
					...
					rrp[0] = ss->ssr*cos(ta2)*cos(ta1);
					rrp[1] = ss->ssr*cos(ta2)*sin(ta1);
					rrp[2] = ss->ssr*sin(ta2);

					// now invert xr, yr, zr into lattice vectors;

					vecmul(rrp, invert_primitive_basis, rw);

					rwx = (int)rw[0] + ss->ssx/2;
					rwy = (int)rw[1] + ss->ssy/2;
					rwz = (int)rw[2] + ss->ssz/2;

				} while (atom_at(rwx, rwy, rwz, ss, se) >= 0);

				//if (atom_at(rwx, rwy, rwz, ss, se) >= 0) goto newt; (made redundant with do while)

				natn = add_atom(rwx, rwy, rwz, atype, NORMAL, se);
			}*/
		}
...
void compute_transition_array(struct SimulationState *ss)
{
	...
	for (i=0;i<ss->rate_cnt;++i)
	{
		if (ss->rate_arr[i].transition_count != 0)
		{
			...
			g_sum_of_rate_populations += ss->rate_arr[i].transition_count;
	...
	/*if (deposition_type == DEPOSITION_TYPE_RAINFALL)
		ss->frequency_sum += (double)ss->ssx*(double)ss->ssy*(deposition_rate_of_a + deposition_rate_of_b + deposition_rate_of_c);
	else if (deposition_type == DEPOSITION_TYPE_RANDOM_WALKER)
		ss->frequency_sum += (4*PI*ss->ssr*ss->ssr)*(deposition_rate_of_a + deposition_rate_of_b + deposition_rate_of_c);*/ //deposition is vestigial
	...
	for (i=0;i<total_lists;++i)
	{
		...
		g_current_probability += ss->rate_arr[x].frequency/ss->frequency_sum;
...
/******************************************************************************/
/******************************************************************************/
// updates [ss->atom_arr[atom_idx], ss->transition_arr[i], ss->rate_arr[i].transition_start_idx], initializes ss->rate_arr
int refresh_transitions(int atom_idx, struct SimulationState *ss, struct SimulationEnv *se) // atom_idx = index on atom list
{
	int i, j;
	int rate_idx; // position of rate rate in rate list ss->rate_arr[]
	...
	for (i=0;i<se->max_neighbors;++i)
	{
        ...
        j = atom_at(next_x, next_y, next_z, ss, se);

		if (j >= 0) ss->atom_arr[atom_idx]->occupied_neighbor_sites[i] = j;
	...
	for (i=0; i<se->max_neighbors; ++i)
	{
		...
		{ // if unoccupied, consider transition
			//printf("final config\n");
			g_final_config_neighbor_cnt = get_final_configuration2(atom_idx, i, end_config, ss, se);
			...
			{ // if rate constant already in rate list, use that rate list index
				add_to_transition_list(rate_idx, atom_idx, i, ss);
			}
			else
			{
	...
	//printf("end stuff\n");
	rate_idx = is_on_transition_list(rate, ss);
...
int is_on_transition_list(double rate, struct SimulationState *ss)
{
	int i;
...
// updates rate_array[ss->rate_cnt], ss->rate_cnt
int create_new_transition(double rate, struct SimulationState *ss)
{
...
// add to ss->rate_arr[rate_idx] the atom atom_idx going in direction offset_idx
// updates ss->transition_arr, ss->rate_arr[rate_idx].transition_count, ss->atom_arr[atom_idx]->transition_indices[offset_idx]
void add_to_transition_list(int rate_idx, int atom_idx, int offset_idx, struct SimulationState *ss) // ss->rate_arr index, ss->atom_arr index, jump_offset index
{
	int i; // loop variable
	int n;
	int initial_transition_index, final_transition_index; // initial and final ss->transition_arr index
	...
	// what is this
	//		final_transition_index = ss->rate_arr[ss->rate_cnt-1].transition_start_idx + ss->rate_arr[ss->rate_cnt-1].number;	
	//		ss->transition_arr[final_transition_index] = (Transition *)malloc(sizeof(Transition));
...
/******************************************************************************/
/******************************************************************************/
// updates ss->atom_arr[atom_idx], ss->transition_arr[i], ss->rate_arr[i].transition_start_idx
void take_off_transition_list(int atom_idx, int offset_idx, struct SimulationState *ss)	// removes atom jumping in the jump_offset[offset_idx] direction
{
	...
	// find out what Rate in ss->rate_arr this is
	...
	// TODO: do these later, after it's been removed and ss->transition_arr has been rearranged
	--ss->transition_cnt;
	...
	if (ss->rate_arr[rate_idx].transition_count == 0) // if list is empty
	{
		...
		{
			--ss->rate_arr[i].transition_start_idx; // move ss->rate_arr offsets of larger indicies down one
			...
			// update the transition index in the corresponding atom in ss->atom_arr to have the new (lower) transition index 
		...
		free(ss->transition_arr[ss->transition_cnt]);			// free up the very last member of the last ss->transition_arr
...
void check_system(struct SimulationState *ss, struct SimulationEnv *se)
{
	...
	for (j=0;j<ss->atom_cnt;++j)
		for (i=0;i<se->max_neighbors;++i)
		{
			...
			k = atom_at(next_x, next_y, next_z, ss, se);
			...
			if (ss->atom_arr[j]->occupied_neighbor_sites[i] == -2)
			{
				...
				for (k=0;k<se->max_neighbors;++k)
				{
					...
					if ((m >= 0)&&(m!= j))
					{
						...
						if (n != -2)
						{
							if (n == -1) ss->atom_arr[m]->occupied_neighbor_sites[opposite_offset[k]] = -2;
							if (n == -3) ss->atom_arr[j]->occupied_neighbor_sites[i] = -3;		// random trumps buried
	...
	/*for (j=0;j<ss->atom_cnt;++j)
	{
		...
		if (k == se->max_neighbors)
		{
			...
			if (j != (ss->atom_cnt-1))
					move_atom((ss->atom_cnt-1), j, ss, se);

			free(atom[ss->atom_cnt-1]);
			--ss->atom_cnt;
			--j;
		}
	}*/
									...
									bool solubility[3])
{
	...
	/*if ((ncsk == 1)||(ss->evaporation_flag == false)) //I don't think this happens
	{
		energy = 1000.;
