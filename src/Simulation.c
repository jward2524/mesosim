        ...
        while (moved_flag == false) {
            if ((transition_type_probability >= ss->transition_probability.lbound[j]) &&
                ...
                selected_transition_idx =
                    ss->rate_arr[k].transition_start_idx +
                    (int)(drandj(&rand_seed) * (double)ss->rate_arr[k].transition_count);
                        ...
                        struct SimulationEnv *se) // atom_idx = index on atom list
{
    ...
    for (int i = 0; i < se->num_energy_contributors; ++i) {
        ...
        if (neighbor_idx >= 0) {
            ...
            int bond_idx =
                get_bond_index(ss->atom_arr[atom_idx]->type, ss->atom_arr[neighbor_idx]->type, se);
        ...
        *ss) // removes atom jumping in the se->transition_vectors[offset_idx] direction
{
    ...
    if (ss->rate_arr[rate_idx].transition_count == 0) // if list is empty
    {
        for (i = rate_idx + 1; i < ss->rate_cnt; ++i) {
            --ss->rate_arr[i]
                  .transition_start_idx; // move rate_arr offsets of larger indicies down one
            ...
            transition_end_idx = ss->rate_arr[i].transition_start_idx +
                                 ss->rate_arr[i].transition_count; // count is always at least 1
